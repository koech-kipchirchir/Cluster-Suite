...
import api, { setAuthToken, userApi } from "../api";
...
export default function WalletTab({ theme, darkMode }) {
  ...
  const handleCustomTopUp = async (wallet) => {
    const amount = parseFloatSafe(topUpAmounts[wallet.id]);
    if (!amount) return;
    await updateWalletBalance(wallet, amount);
    ...
    <div style={{ animation: "fadeIn 0.3s ease" }}>
      <div style={{ marginBottom: "24px", display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: "16px" }}>
        ...
        <div style={{ background: theme.card, padding: "24px", borderRadius: "20px", border: `1px solid ${theme.border}` }}>
          ...
          <div style={{ fontSize: "32px", fontWeight: "900" }}>${totalTarget.toLocaleString()}</div>
        </div>
        <div style={{ background: theme.card, padding: "24px", borderRadius: "20px", border: `1px solid ${theme.border}` }}>
            ...
            return (
              <div key={wallet.id} style={{ background: theme.card, padding: "24px", borderRadius: "24px", border: `1px solid ${theme.border}`, boxShadow: darkMode ? "0 20px 25px -5px rgba(0,0,0,0.4)" : "0 10px 15px -3px rgba(0,0,0,0.05)" }}>
                ...
                <div style={{ display: "flex", gap: "8px", marginBottom: "16px" }}>
                  {["10", "50", "100"].map((amt) => (
                    <button key={amt} onClick={() => updateWalletBalance(wallet, amt)} disabled={updatingWalletId === wallet.id} style={{ flex: 1, padding: "10px", borderRadius: "10px", border: `1px solid ${theme.border}`, background: theme.card, color: theme.text, cursor: "pointer", fontWeight: "700", opacity: updatingWalletId === wallet.id ? 0.7 : 1 }}>
